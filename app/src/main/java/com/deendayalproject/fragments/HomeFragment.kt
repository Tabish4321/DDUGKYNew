...
class HomeFragment : BaseFragment<FragmentHomeBinding>(
        ...
        binding.navigationView.setNavigationItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_share -> {
                    ...
                    val shareText = """
                                Check out DDUGKY app on Google Play:

                                https://play.google.com/store/apps/details?id=$packageName""".trimIndent()
    ...
    private fun updateExpansionUI(binding: ItemModuleBinding, isExpanded: Boolean) {
        ...
        binding.ivExpandArrow.animate().rotation(if (isExpanded) 180f else 0f).setDuration(250)
            .start()
    }
...
//        }
    ...
    private fun handleFormClick(form: Form) {
        when (form.formCd) {
            "TRAINING_CENTER_APP" -> navigate(R.id.action_homeFragment_to_centerFragment)

            "RESIDENTIAL_FACILITY_FORM" -> navigate(R.id.action_homeFragment_to_rfCenterFragment)
...
//                navigate(R.id.action_homeFragment_to_SelectionSrlm)
    ...
    private fun fetchModules() {
        ...
        val request = ModulesRequest(
            loginId = loginId, appVersion = BuildConfig.VERSION_NAME
        )
    ...
    override fun loadInitialData() {
        if (AppUtil.getSavedLoginIdPreference(requireContext()) == BuildConfig.USER_NAME_FOR_APP) {
            val action = HomeFragmentDirections.actionHomeFragmentToQTeamFormFragment(
                "1", "DDUGKY Training Center", "S2025"
            )
